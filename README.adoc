...
```
stream create --deploy true --name demo --definition "time | scripty --script=\"return payload+'::'+payload\" | log"
```

```
stream create --deploy true --name demo2 --definition "time | scripty --script=\"function double(a) { return a+''+a;} double(payload);\" --language=js | log"
```